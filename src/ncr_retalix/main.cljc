(ns ncr_retalix.main
  "Kotodama WASM entrypoint for the Ncr_retalix clean-room actor (L4) — Clojure port.
  ...
  facts (`ncr_retalix.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ncr_retalix")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "ncrretal_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "ncrretal_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {} :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "ncrretal_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"ncr_retalix.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ncr_retalix-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ncr_retalix-compat/ready)
